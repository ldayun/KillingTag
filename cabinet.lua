...
function scene:create( event )
	...
	local background = display.newImageRect("image/P.E/cabinet.jpg", display.contentWidth, display.contentHeight)
	...
	function lock_click ( event )
		-- body
		if (event.x >= 770 and event.x <= 868 and event.y >= 316 and event.y <=427) then
			print("lock 확대")
			--composer.showOverlay('locked')
			composer.gotoScene("ff1.lock")
		end
	end
	self.view:addEventListener("tap", lock_click)
