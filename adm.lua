...
function scene:create( event )
	...
	local background = display.newImageRect("image/adm/bg.jpg", display.contentWidth, display.contentHeight)
	...
	local content_adm = display.newText("행정실 대사 들어갈곳", display.contentWidth*0.5, display.contentHeight*0.88)
	content_adm.size = 25

	--json 불러오기
	local Data = jsonParse("json/text_adm.json")
	if Data then
		print(Data[1].dialog)
	end
	...
	function nextScript( event )
		...
		if(index > #Data) then 
			...
			content_adm.text = Data[index].content
		end
	end
	...
	function click( event )
		...
		if (cctv_flag == 0) then
			if (event.x >= 780 and event.x <= 1040 and event.y >= 270 and event.y <= 380) then
				print("씨씨티비 꺼야됨")
				composer.gotoScene("ff1.cctv_bf")
			end
		else
			if (event.x >= 780 and event.x <= 1040 and event.y >= 270 and event.y <= 380) then
				print("이미 끔")
