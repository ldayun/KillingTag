...
function scene:create( event )
	...
	local background = display.newImageRect("image/P.E/bg.jpg", display.contentWidth, display.contentHeight)
	...
	local content_PE = display.newText("여기는 어떻게 변하질 않냐...", display.contentWidth*0.5, display.contentHeight*0.88)
	content_PE.size = 25
	...
	function locker_click ( event )
		-- body
		if (event.x >= 755 and event.x <= 1135 and event.y >= 30 and event.y <=525) then
			print("locker 보기")
			--composer.showOverlay('locked')
			composer.gotoScene("ff1.cabinet")
		end
	end
	self.view:addEventListener("tap", locker_click)

	sceneGroup:insert(background)
	sceneGroup:insert(setting)
	sceneGroup:insert(back)
	sceneGroup:insert(box)
	sceneGroup:insert(content_PE)
