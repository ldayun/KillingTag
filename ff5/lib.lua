...
function scene:create( event )
	...
	function itemShelf:tap(event)
		composer.gotoScene("ff5.itemShelf")
	end
	function otherShelves ( event )
		if (event.x >= 100 and event.x <= 340 and event.y >= 60 and event.y <= 500 
		or event.x >= 480 and event.x <= 1080 and event.y >= 100 and event.y <= 500) then
